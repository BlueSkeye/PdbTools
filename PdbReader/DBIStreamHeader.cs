...
namespace PdbReader
{
    ...
    internal struct DBIStreamHeader
    {
        ...
        internal uint Magic;
        /// <summary>this value always appears to be V70, and it is not clear what the other values are for.</summary>
        internal StreamVersion VersionHeader;
        /// <summary>The number of times the PDB has been written. Equal to the same field from the PDB
        /// Stream header.</summary>
        internal uint Age;
        /// <summary>The index of the Global Symbol Stream, which contains CodeView symbol records for all
        /// global symbols. Actual records are stored in the symbol record stream, and are referenced from
        /// this stream.</summary>
        internal ushort GlobalStreamIndex;
        /// <summary>A bitfield containing values representing the major and minor version number of the
        /// toolchain (e.g. 12.0 for MSVC 2013) used to build the program. For bit layout
        /// <see cref="GetMajorVersion()"/>, <see cref="GetMinorVersion()"/> and
        /// <see cref="IsNewVersionFormat()"/></summary>
        internal ushort BuildNumber;
        /// <summary>The index of the Public Symbol Stream, which contains CodeView symbol records for all
        /// public symbols. Actual records are stored in the symbol record stream, and are referenced from
        /// this stream.</summary>
        ...
        /// <summary>The stream containing all CodeView symbol records used by the program. This is used for
        /// deduplication, so that many different compilands can refer to the same symbols without having to
        /// include the full record content inside of each module stream.</summary>
